...
class TransparantImagesKristikController extends Controller {
  const MODEL = "App\Models\TransparantImageKristik";
  ...
  public function imageTransparant($background, $im, $square,$keyColors, $newColors)
  {
  ...
  function createTransparantImage(Request $request){
    // $data = base64_decode($request->file('img_file'));
    ...
    if ($klength>=2) {
        ...
        return $this->imageTransparant($background, $im, $square,$twoKeys, $newColors);
    }else
    {
      $model = self::MODEL;
      $resultResponse = array();
